...
import { Calendar, Edit, Trash2, Eye, Zap, Star, TrendingUp } from "lucide-react";
import Link from "next/link";

export default function NewsTableRow({ news, index, currentPage, itemsPerPage, isDark, onEdit, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {news.isBreaking && (
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-red text-white text-[10px] font-bold rounded">
      ...
      <td className="px-6 py-4">
        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(news.status)}`}>
          {news.status || 'draft'}
        </span>
       </td>
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          <Link
            href={`/admin/news/manage/${news.id}`}
          ...
          <button
            onClick={() => onDelete(news.id, news.title, news.image)}
            className={`p-2 rounded-lg transition-all duration-200 cursor-pointer ${
              isDark
